...
###   starshipshacore.org and starshipsha.org and compares them.
...
WORKINGDIR="/tmp/starshipsha_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://starshipshacore.org"
HOST2="https://starshipsha.org"
BASEDIR="/bin/"
VERSIONPREFIX="starshipsha-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'starshipsha-',
...
if [ $? -ne 0 ]; then
   echo "starshipsha.org failed to provide signature file, but starshipshacore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "starshipsha.org and starshipshacore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Starshipsha Core binary release signing key installed?"
   fi
