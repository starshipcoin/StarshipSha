...
The RPM spec file provided here is for Starshipsha-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://starshipsha.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Starshipsha git repository](https://github.com/starshipsha/starshipsha) but are not part of
...
Currently only `Source10` is used, for the example `starshipsha.conf` file.
...
in the starshipsha git repository. At present only an SVG version of the Starshipsha
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the starshipsha
...
needed when 0.12.1 is released, a proper fix is already in the Starshipsha git
master branch.
...
idea to build Starshipsha Core against a static version of that library compiled
...
The `starshipsha.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `starshipsha-server` package. At this point, most RPM based
...
The `starshipsha-server` package should have SELinux support. How to properly do
...
Starshipsha-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/starshipsha/). He has been
packaging Starshipsha for Fedora at least since 2012.
...
4. I renamed the `starshipsha` package that contains the Qt GUI to `starshipsha-core` as
that appears to be how the general population refers to it, in contrast to
`starshipsha-xt` or `starshipsha-classic`. I wanted to make sure the general population
