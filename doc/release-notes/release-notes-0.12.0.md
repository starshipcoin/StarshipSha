bitcoin Core version 0.12.0 is now available from:
  ...
  <https://starshipsha.org/bin/starshipsha-core-0.12.0/>
...
[libsecp256k1](https://github.com/starshipsha-core/secp256k1) instead of OpenSSL.
...
(https://github.com/starshipsha/bips/blob/master/bip-0130.mediawiki)
...
(https://github.com/starshipsha/bips/blob/master/bip-0125.mediawiki).
...
This allows running starshipshad without having to do any manual configuration.
...
`starshipsha.conf`, where `N` is the number of MiB to allot for
raw block & undo data.
...
notes of v0.11.0](https://github.com/starshipsha/starshipsha/blob/v0.11.0/doc/release-notes.md#block-file-pruning).

`NODE_BLOOM` service bit
------------------------

Support for the `NODE_BLOOM` service bit, as described in [BIP
111](https://github.com/starshipsha/bips/blob/master/bip-0111.mediawiki), has been
...
* OP_NOP2 has been renamed to OP_CHECKLOCKTIMEVERIFY by [BIP 65](https://github.com/starshipsha/bips/blob/master/bip-0065.mediawiki)
...
Then, to tunnel a SSL connection on 217785 to a RPC server bound on localhost on port 18332 do:

    stunnel -d 217785 -r 127.0.0.1:18332 -p stunnel.pem -P ''
...
caching. A sample config for apache2 could look like:
    ...
    <Location /starshipsharpc>
        ProxyPass http://127.0.0.1:17785/
        ProxyPassReverse http://127.0.0.1:17785/
        # optional enable digest auth
        # AuthType Digest
        # ...

        # optional bypass starshipshad rpc basic auth
        # RequestHeader set Authorization "Basic <hash>"
        # get the <hash> from the shell with: base64 <<< starshipsharpc:<password>
    </Location>
...
Restarting starshipshad will no longer clear out the list of banned peers; instead
...
- #6269 `d4565b6` gitian: Use the new starshipsha-detached-sigs git repo for OSX signatures (Cory Fields)
...
- #6600 `ef4945f` Include starshipsha-tx binary on Debian/Ubuntu (Zak Wilcox)
...
- #6390 `6a73d66` tests: Fix starshipsha-tx signing test case (Wladimir J. van der Laan)
...
- #6730 `cb4d6d0` build: Remove dependency of starshipsha-cli on secp256k1 (Wladimir J. van der Laan)
...
- #6387 `11576a5` [starshipsha-cli] improve error output (Jonas Schnelli)
- #6401 `6db53b4` Add BITCOIND_SIGTERM_TIMEOUT to OpenRC init scripts (Florian Schmaus)
- #6430 `b01981e` doc: add documentation for shared library libstarshipshaconsensus (Braydon Fuller)
...
- #6504 `b6fee6b` Rationalize currency unit to "STAS" (Ross Nicoll)
